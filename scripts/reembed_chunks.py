...
Invocation (must run inside the hlh_api container so env + Python path
match prod exactly):
    docker exec -i hlh_api python3 -m scripts.reembed_chunks
"""
...
async def main() -> int:
    ...
    if not embedding_model:
        logger.error(
            "EMBEDDING_MODEL env var is not set. "
            "Set it to the configured embedding model name before running this script."
        )
        return 1
