...
export function NotesPanel({ dawId }) {
  ...
  const { data: notes = [], isLoading } = useQuery({
    queryKey: ['notes', dawId],
    queryFn: () => listNotes(dawId),
  ...
  const persist = useCallback(async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: ['notes', dawId] })
  }, [selectedId, dawId, localTitle, localContent, queryClient])
  ...
  }, [localTitle, localContent, selectedId, dawId, persist])
  ...
  const createMut = useMutation({
    mutationFn: () => createNote(dawId, { content: '', source_type: 'manual' }),
    onSuccess: async (row) => {
      await queryClient.invalidateQueries({ queryKey: ['notes', dawId] })
    ...
    <aside className="flex h-full min-h-0 w-full min-w-0 flex-col border-l border-sidebar-border bg-sidebar text-sidebar-foreground">
      <div className="border-b border-sidebar-border">
        <div className="flex min-h-16 w-full items-center justify-between gap-2 overflow-hidden px-2 py-2">
          ...
          <Button
            type="button"
            className="fs-nav h-8 shrink-0 px-2"
            variant="secondary"
            size="sm"
            disabled={!dawId || createMut.isPending}
      ...
      <ScrollArea className="min-h-0 flex-[3]">
        <div className="flex flex-col gap-0.5 p-2 pb-2">
          {!dawId ? (
            <p className="fs-nav px-1 text-muted-foreground">Open a notebook to see notes.</p>
          ) : isLoading ? (
