...
- [ ] On `hlh_infer` (folder B's service, read_only), set `HOME=/cache` in the
...
- [ ] In `hlh_orchestra/bootstrap.py`, add `ensure_infer_cache_ownership()`
...
- [ ] Map `GatedRepoError` / 401-equivalent to the existing
      "License acceptance required. Visit {license_url} ..." message.

### C1.6 - Branch pull_model on kind
- [ ] In `pull_model` (`model_puller.py:460`), look up the spec in
      `MODEL_REGISTRY` by `(role, tier, model_id)` to derive `kind`.
- [ ] For `kind == "snapshot"`: skip `_check_disk_space` and sha256; mark
...
- [ ] Keep the existing file path untouched for `kind == "file"`.
- [ ] `python -m py_compile backend/services/model_puller.py`.

### C1.6a - Fix the pre-existing ALL_ROLES gap (add "tasks")
- [ ] In `backend/services/model_puller.py:36`, add `"tasks"` to `ALL_ROLES`
      ...
      `tasks`).
- [ ] `python3 -m py_compile backend/services/model_puller.py`.
...
- [ ] After a boot, assert the registry rows via the Models API:
...
- [ ] In `docker-compose.yml`, add `hlh_infer_cache:/cache` to the `hlh_api`
...
- [ ] In `backend/services/bundled_providers.py:37,40,44` set
      `BUNDLED_CHAT_BASE_URL`, `BUNDLED_EMBED_BASE_URL`, `BUNDLED_RERANK_BASE_URL`
      all to `"http://hlh_swap:9620"`.
- [ ] Leave `BUNDLED_EMBED_MODEL = "qwen3-embed"` and
      `BUNDLED_RERANK_MODEL = "qwen3-reranker"` unchanged.
- [ ] `python -m py_compile backend/services/bundled_providers.py`.

### C2.2 - Verify self-heal on boot
- [ ] After boot, assert the three bundled provider rows resolve
...
- [ ] Read `rag.py:251-278`; confirm `documents` is `[p["text"] for p in passages]`
      (list of strings) and the parse reads `results[].index` + `relevance_score`.
      If any divergence, fix to that exact shape.
- [ ] Keep `return_documents: false` and the broad `except Exception` soft-fallback
      returning `None`.
- [ ] `python -m py_compile backend/services/rag.py`.

### C3.2 - Live rerank probe through the front-door
- [ ] `docker exec hlh_api python -c "import asyncio,httpx; ..."`:
...
- [ ] Confirm no code change needed beyond the provider `base_url` (C2); the
...
- [ ] In `backend/routers/sources.py:454-484`, extract the body into a plain
...
- [ ] Behavior identical (delete chunks, mark processing,
      `asyncio.create_task(_ingest_source(...))`, return counts).
- [ ] `python -m py_compile backend/routers/sources.py`.
...
- [ ] New `backend/services/embed_cutover.py` with
      `async def run_embed_cutover(conn_or_pool) -> None`.
- [ ] No-op if `global_settings['embed_cutover_boofinity_done']` exists.
- [ ] No-op (without setting sentinel) if `system_profile.tier = 'external'`.
- [ ] Readiness gate: embed `bundled_models` row `status='ready'` AND a live
      `/v1/embeddings` probe through the front-door returns a 1024-vector; on
      failure return WITHOUT setting the sentinel.
- [ ] On ready: `INSERT ... ON CONFLICT (key) DO NOTHING` the sentinel with an
...
- [ ] Add a completion hook in the ingest path (the tail of `_ingest_source` in
...
- [ ] In `backend/main.py` lifespan, after `apply_bundled_bindings` and
...
- [ ] Add `retrieval_rebuilding` to the system/settings status payload the
...
- [ ] Boot once with a ready embed backend; confirm via the sources API that
      `queued > 0` and the sentinel is set (`docker exec hlh_api python -c ...`).
- [ ] Restart; confirm the cutover no-ops (no new reingest; `queued` not re-fired)
...
- [ ] Add `_check_infer_cache_writable()` mirroring `_check_models_writable`
      (`doctor.py:418`) but probing `INFER_CACHE_DIR` (default `/cache`); ERROR
      with the `chown -R 1000:1000 /cache` remedy.
- [ ] Register it in `run_checks()` (`doctor.py:480`) next to
      `_check_models_writable()`.
- [ ] `python -m hlh.doctor` (note: exits 1 if any check ERROR by contract); use
      `|| true` to capture output.

### C6.2 - Confirm model_pulls covers snapshots
- [ ] Confirm `_check_model_pulls` (`doctor.py:441`) needs no change: a `failed`
...
- [ ] Confirm `backend/routers/providers.py:374-375` still reads
...
- [ ] Create `backend/scripts/verify_boofinity_embed_rerank.sh`: front-door
...
- [ ] In `verify_embedding_reranker_settings.sh` and
...
- [ ] In `verify_bundled_immutability.sh`, update the asserted bundled-row
...
- [ ] Deploy this folder **with or after** folder B. Confirm the doctor
...
- [ ] Update `CHANGELOG.md` `[Unreleased]` (AI track) by hand or a targeted
