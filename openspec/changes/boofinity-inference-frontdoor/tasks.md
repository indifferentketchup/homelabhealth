...
- [x] Create `hlh_swap/config.yaml` with a `models:` map per design.md. Define
...
- [x] Define `qwen3-embed`, `qwen3-reranker`, `qwen3-vl-embed`, `qwen3-vl-rerank`
...
- [x] Add a `macros:` block for `llama_cmd` and `boof_cmd` and reference them via
      `${llama_cmd}` / `${boof_cmd}` in each model's `cmd:`.
- [x] Set the top-level `healthCheckTimeout: 120` and `startPort: 5800`.
- [x] Confirm there is NO Docker socket reference, NO `docker start`/`docker
...
- [x] In `hlh_swap/config.yaml` add a `groups:` entry `vram_constrained` with
...
- [x] Confirm the `cmd` / `ttl` / `healthCheckTimeout` / `groups` (`swap`,
...
- [x] Copy `hlh_swap/config.yaml` to `hlh_orchestra/templates/swap_config.yaml`
      byte-for-byte.
- [x] In `hlh_orchestra/bootstrap.py` (near `MODELS_INI_TEMPLATE` at line 57 and
...
- [x] Create `hlh_swap/Dockerfile` as a multi-stage build per design.md: FROM the
...
- [x] Document the CUDA variant: same Dockerfile with
...
- [x] In `hlh_chat/models.ini`, delete the `[qwen3-embed]` section (lines 49-57)
      and the `[qwen3-reranker]` section (lines 59-66), including their leading
      comment blocks.
- [x] Confirm `[medgemma]`, `[gemma-tasks]`, `[qwen-chat]`, and `[*]` remain;
...
- [x] In `hlh_orchestra/templates/models.ini`, delete the same two sections if
      present.
...
- [x] Remove the standalone `hlh_chat_cpu`/`hlh_chat_gpu` and any `hlh_infer`
      service definitions; their work now runs as child processes of `hlh_swap`.
- [x] Add an `x-hlh-swap-base` anchor (top-level, like the former
      ...
      Docker socket.
- [x] Add `hlh_swap_cpu` (`<<: *hlh-swap-base`, image
      `${HLH_SWAP_IMAGE:-ghcr.io/indifferentketchup/hlh-swap:0.1.0-cpu}`,
      `profiles: [bundled]`).
- [x] Add `hlh_swap_gpu` (`<<: *hlh-swap-base`, image `...:0.1.0-cuda`,
...
- [x] Keep `read_only: true` on `hlh_swap`. The child-process topology needs no
...
- [x] In the boofinity child's `cmd:` (the `boof_cmd` macro in
      ...
      flag, NOT an env var.
- [x] Pass `--dtype ${HLH_INFER_DTYPE:-float32}` (Pascal-safe default) on the
...
- [x] Confirm the combined image ships `python` for the healthcheck probe (it is
      FROM the boofinity base, which has python); the probe uses `urllib`, no
      wget/curl dependency.
- [x] Confirm llama-swap's v226 readiness route on port 9620 (`/v1/models` vs a
      dedicated readiness endpoint); align the `hlh_swap` healthcheck `test:` to
      the confirmed path.
- [x] Confirm the boofinity child health route under `--url-prefix /v1`
...
- [x] In the `volumes:` block (`docker-compose.yml:242-248`) add
...
- [x] `docker compose --profile bundled config -q` exits 0 (no schema errors).
- [x] `docker compose --profile bundled-gpu config -q` exits 0.
- [x] Confirm no host ports and no Docker socket on `hlh_swap`:
...
- [x] In `backend/services/image_config.py:18`, alongside `LLAMA_CPP_VERSION`,
      add `LLAMA_SWAP_VERSION = "v226"` and `BOOFINITY_VERSION = "0.1.0"`.
- [x] Extend `_MANAGED_KEYS` (line 84) to include `"HLH_SWAP_IMAGE"`,
...
- [x] In `write_tier_env`, write `HLH_INFER_DTYPE` with the default value
...
- [x] Add `swap_image: str` and `infer_mem: str` fields to the `TierImages`
      dataclass (`image_config.py:22-27`); collapse the old `chat_image` /
      `infer_image` fields into `swap_image`.
- [x] Populate `TIER_IMAGE_MAP` (lines 30-79): `swap_image` =
...
- [x] In `write_tier_env` (`image_config.py:126-131`), add `HLH_SWAP_IMAGE`,
      `HLH_INFER_MEM`, and `HLH_INFER_DTYPE` (default `"float32"`) to the
      `managed` dict from the tier's `TierImages`.
- [x] Update the closing `logger.info` (lines 161-164) to include the new values.
...
- [x] Create `resource_policy.py` with a frozen `TierPolicy` dataclass
...
- [x] Expose `policy_for(tier)`, `coresident(tier)`, `gemma_degradation(tier)`.
      Per ADR-0002: `gpu-4gb` -> `unavailable`; `cpu-*`/`apple-mlx` ->
      `offload_cpu`; `gpu-24gb+` -> resident (non-exclusive group).
- [x] No DB / HTTP / asyncio imports - pure data + functions.
- [x] Ensure every `TierPolicy` field has a real in-scope consumer (no dead
...
- [x] Confirm `hlh_swap/config.yaml` is the single static exclusive group for v1
...
- [x] In `pipeline_status._estimate_key` (`backend/services/pipeline_status.py:24-33`)
      add a `"swapping": "estimate_ms_swap"` entry to the dict.
- [x] Add a helper (e.g. `infer_backend_state(model)`) that GETs the front-door
...
- [x] In `backend/hlh/doctor.py:run_checks` (lines 470-491) add
      `await _check_sidecar("hlh_swap", "http://hlh_swap:9620/v1/models")`.
- [x] Add a boofinity-child check probing the boofinity child's readiness through
...
- [x] In `_check_image_tier_match` (`doctor.py:389-415`) read
...
- [x] Add `_check_embed_rebind_consistency()` to `doctor.py` and register it in
      ...
      `hlh_swap:9620`.
- [x] `python3 -m py_compile backend/hlh/doctor.py` exits 0.
...
- [x] Confirm folder A's `TIER_IMAGE_MAP` rewrite drops `vision` from
...
- [x] In `backend/scripts/verify_a1_5_hardening.sh`, replace the standalone
...
- [x] Replace the exact `4294967296` `mem_limit` assertion with a positive-value
      check for `hlh_swap` (`[[ "$swap_mem" -ge 1073741824 ]]`), matching the
      tier-scaled `HLH_INFER_MEM`.
- [x] Keep the no-host-ports assertion for `hlh_swap`. Add a check that no
...
- [x] `python3 -m py_compile $(find backend -name '*.py')` produces no errors.
- [x] `docker compose --profile bundled config -q` and
      `docker compose --profile bundled-gpu config -q` both exit 0.
- [x] `diff hlh_swap/config.yaml hlh_orchestra/templates/swap_config.yaml` clean.
- [x] `grep -c 'qwen3-embed\|qwen3-reranker' hlh_chat/models.ini hlh_orchestra/templates/models.ini` returns 0 for both.
- [x] No Docker socket anywhere: `grep -rc 'docker.sock' docker-compose.yml hlh_swap/config.yaml` returns 0 for both.
- [x] Update `CHANGELOG.md` under `[Unreleased]` (AI / Tooling tracks) with the
      combined front-door, child-process backends, resource policy, and
      doctor-check entries.
- [x] In the same CHANGELOG entry note the `HLH_INFER_DTYPE` default of
