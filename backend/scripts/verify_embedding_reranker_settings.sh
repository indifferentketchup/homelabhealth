...
HOST_TAILNET_IP="${HOST_TAILNET_IP:-localhost}"
MOCK_PORT="${MOCK_PORT:-9611}"

INFINITY_EMB_URL="${INFINITY_EMB_URL:-}"
INFINITY_RERANK_URL="${INFINITY_RERANK_URL:-}"

if [[ -z "$INFINITY_EMB_URL" || -z "$INFINITY_RERANK_URL" ]]; then
    echo "SKIP: INFINITY_EMB_URL and INFINITY_RERANK_URL must both be set."
    exit 0
fi
